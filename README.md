...
4. Open **Quests** and start **The hidden gem**. It generates a campus outfit containing a less-worn garment from the sample closet.
...
| Styling | Deterministic metadata rules; top + bottom or dress, plus shoes; optional presentation outerwear; only stored clothes |
...
The frontend uses React and Vite with no backend or API keys. Garment illustrations are original inline SVGs. Google Fonts is optional; system sans-serif fonts work when offline. The lockfile pins dependencies for repeatable installs.
