...
export function rankOutfits(items, context, { limit = 3, requireLowUse = false } = {}) {
  ...
  const k = Math.max(1, Math.min(3, Math.floor(limit) || 3));
  const pool = category => {
    const sorted = usable.filter(i => i.category === category).sort((a,b) => a.wears - b.wears || a.id.localeCompare(b.id));
    // For each fixed outfit shape the score is additive with a fixed denominator.
    // An item below rank k cannot enter the global top k: k better replacements
    // exist in the same slot. Keep k low-use replacements too for quest feasibility.
    return [...new Map([...sorted.slice(0,k), ...(requireLowUse ? sorted.filter(i => i.wears <= 1).slice(0,k) : [])].map(i => [i.id,i])).values()];
  };
